...
const Partners = () => {
  ...
  useEffect(() => {
    ...
    setPartnersData([
      { partnerNo: 'Partner 1', requestedMeal: 'Meal Package 1', orderStatus: 'Preparing', assignedPartner: 'XYZ Catering', choosePartner: 'Choose' },
      { partnerNo: 'Partner 2', requestedMeal: 'Meal Package 2', orderStatus: 'Pending', assignedPartner: 'ABC Catering', choosePartner: 'Choose' },
    ]);

    setPartnershipRequestData([
      { companyName: 'We Eats', image: '', address: '009, 4th St. Tenejero', action: 'Accept' },
      { companyName: 'PaBliss', image: '', address: '121, Milan St.', action: 'Decline' },
    ]);

    setRegisteredPartnersData([
      { companyName: 'ABC Catering', image: '', address: '364, 4th St. London', email: 'abcfood@gmail.com' },
      { companyName: 'XYZ Catering', image: '', address: '100, Milan St.', email: 'xyzmeal@gmail.com' },
    ]);
  }, []);
    ...
    <Layout>
      <div className="flex min-h-screen mr-5">
        ...
        <div className="flex-1 p-4">
          ...
          <h1 className="text-3xl font-bold mb-5 mt-10 text-center">Partners</h1>
          ...
          <table className="min-w-full bg-white border border-gray-300 mb-8">
            <thead className="bg-blue-800 text-white">
              <tr>
                ...
                <th className="py-2 px-4 border-b font-medium">Requested Meal</th>
                <th className="py-2 px-4 border-b font-medium">Order Status</th>
                <th className="py-2 px-4 border-b font-medium">Assigned Partner</th>
                <th className="py-2 px-4 border-b font-medium">Choose Partner</th>
              </tr>
            </thead>
            <tbody className="text-center">
              {partnersData.map((partner, index) => (
                <tr key={index}>
                  ...
                  <td className="py-2 px-4 border-b">{partner.requestedMeal}</td>
                  <td className="py-2 px-4 border-b">
                    <span
                      className={`inline-block px-3 py-1 rounded-full ${partner.orderStatus === 'Preparing'
                  ...
                  <td className="py-2 px-4 border-b">{partner.assignedPartner}</td>
                  <td className="py-2 px-4 border-b">
                    <select
                      value={partner.choosePartner}
                      onChange={e => handleChoosePartnerChange(index, e.target.value)}
            ...
            <tbody className="text-center">
              {partnershipRequestData.map((request, index) => (
                <tr key={index}>
                  ...
                  <td className="py-2 px-4 border-b">
                    <span
                      className={`inline-block px-3 py-1 rounded-full ${request.action === 'Accept'
                          ? 'bg-green-500 text-white'
                          : 'bg-red-500 text-white'
                        }`}
                    >
